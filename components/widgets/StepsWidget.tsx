...
export function StepsWidget({
  ...
  completedSteps = 3,
  totalSteps = 7,
  highlight = 'Produtividade em alta'
}: StepsWidgetProps) {
  const percentage = Math.max(0, Math.min(100, Math.round((current / Math.max(total, 1)) * 100)));
  ...
  return (
    <Card className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white ring-1 ring-white/10">
      ...
      <CardHeader className="relative z-10 flex flex-row items-center justify-between">
        ...
        <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-white/80">{metaLabel}</span>
      </CardHeader>
